...
export function VatLifecycleView({ user, workspace, profile, onNavigate }: { user: AuthUser | null; workspace: BusinessWorkspace; profile: BusinessProfile; onNavigate: (view: ViewId) => void }) {
  ...
  const steps: Array<{ title: string; detail: React.ReactNode; done: boolean; view: ViewId; action: string }> = [
    ...
    { title: "Complete business identity", detail: /^\d{9}$/.test(profile.tin) ? <>TIN <span className="mono">{profile.tin}</span> saved</> : "Add entity, contact, TIN and IRD e-Services status.", done: /^\d{9}$/.test(profile.tin) && Boolean(profile.address), view: "business", action: "Business profile" },
  ...
  return (
    <>
      ...
      <section className="card pad lifecycle-card">
        <div className="card-head"><div><h2>End-to-end workflow</h2><p>ComplyPilot prepares each step; government registration and final filing remain authorised human actions.</p></div><span className="tag brand"><span className="mono">{steps.filter((item) => item.done).length}/{steps.length}</span> ready</span></div>
