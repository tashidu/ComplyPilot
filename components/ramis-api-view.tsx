  ...
  return <>
    ...
    <div className="grid two ramis-grid">
      <article className="card pad">
        <div className="card-head"><div><h2>Onboarding record</h2><p>Metadata only—no password, JWT or taxpayer secret is stored.</p></div><strong className="mono">{checks.filter(Boolean).length}/{checks.length}</strong></div>
        <div className="workspace-form-grid">
          ...
          <label><span>IRD integration / ticket reference</span><input className="mono" value={value.integrationReference} onChange={(event) => setValue({ ...value, integrationReference: event.target.value })} /></label>
          ...
          <label className="span-two"><span>SSID supplied by IRD (identifier only)</span><input className="mono" value={value.ssid} onChange={(event) => setValue({ ...value, ssid: event.target.value })} placeholder="Never enter the SSID password here" /></label>
