...
export function VatLedgerView({ workspace, profile, period, onSave, onDelete, onCreateInvoice, prefill, extraction, extractionMode }: { workspace: BusinessWorkspace; profile: BusinessProfile; period: VatPeriodRecord; onSave: (value: VatTransactionDraft) => Promise<boolean>; onDelete: (transaction: VatTransaction) => Promise<boolean>; onCreateInvoice: () => void; prefill?: { token: string; fields: Record<string, unknown> } | null; extraction?: InvoiceExtraction | null; extractionMode?: "LIVE_QWEN" | "DEMO_FALLBACK" }) {
  ...
  return <>
    ...
    <div className="grid two ledger-grid">
      <article className="card pad"><div className="card-head"><div><h2>Add transaction</h2><p>Amounts are recomputed on the server; AI cannot change the arithmetic.</p></div><span className="tag brand">Schedule <span className="mono">{schedule}</span></span></div>
        ...
        <div className="calculation-strip"><span>Net <b className="mono">{money(calculation.netAmountLkr)}</b></span><i>+</i><span>VAT {calculation.statedVatVariance ? "as charged" : `${calculation.vatRate}%`} <b className="mono">{money(calculation.vatAmountLkr)}</b></span><i>=</i><span>Gross <b className="mono">{money(calculation.grossAmountLkr)}</b></span></div>
        {calculation.statedVatVariance ? <p className="stated-vat-variance">The supplier charged <b className="mono">{money(calculation.statedVatVariance.statedLkr)}</b> where <b className="mono">{money(calculation.statedVatVariance.expectedLkr)}</b> was expected on this net value. Only what was charged is claimable — the {money(Math.abs(calculation.statedVatVariance.differenceLkr))} difference is recorded for you to query with the supplier.</p> : null}
      ...
      <article className="card pad"><div className="card-head"><div><h2>Current-period records</h2><p>{transactions.length} record{transactions.length === 1 ? "" : "s"} mapped to VAT schedules.</p></div></div><div className="ledger-list">{transactions.map((item) => <div key={item.id}><span className={`transaction-kind ${item.kind === "OUTPUT" ? "output" : "input"}`}>{item.kind === "OUTPUT" ? "OUT" : "IN"}</span><section><strong><span className="mono">{item.invoiceNumber}</span> · {item.counterpartyName}</strong><small><span className="mono">{item.invoiceDate}</span> · Schedule <span className="mono">{item.scheduleCode}</span> · {VAT_TREATMENT_LABELS[item.treatment]}{item.source === "DOCUMENT_EXTRACTION" ? " · read by Qwen" : item.source === "GENERATED_INVOICE" ? " · from a generated invoice" : ""}</small></section><aside><strong className="mono">{money(item.vatAmountLkr)}</strong><small>VAT</small></aside>{item.source === "GENERATED_INVOICE" ? <button className="ledger-remove" disabled title="This line came from a generated tax invoice. Void the invoice in the register to withdraw it." aria-label={`${item.invoiceNumber} is controlled by its tax invoice`}>×</button> : <button className="ledger-remove" disabled={periodClosed} title={periodClosed ? "This period is closed." : `Remove ${item.invoiceNumber} from the ledger`} aria-label={`Remove ${item.invoiceNumber}`} onClick={() => void onDelete(item)}>×</button>}</div>)}{!transactions.length ? <p className="empty-state">No VAT transactions recorded for this period.</p> : null}</div></article>
    </div>
  </>;
}
