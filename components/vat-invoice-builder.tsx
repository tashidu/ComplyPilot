...
export function VatInvoiceBuilder({ profile, period, generated, onSave, prefill }: { profile: BusinessProfile; period: VatPeriodRecord; generated: GeneratedVatInvoice[]; onSave: (value: VatInvoiceDraft) => Promise<GeneratedVatInvoice | null>; prefill?: { token: string; fields: Record<string, unknown> } | null }) {
  ...
  return <>
    ...
    <div className="invoice-builder-grid">
      <article className="card pad invoice-form no-print"><div className="card-head"><div><h2>Invoice details</h2><p>Estimated number: <span className="mono">{previewNumber}</span></p></div><span className="tag brand">{value.treatment === "STANDARD_18" ? "18% VAT" : "ZERO-RATED"}</span></div>
...
function InvoicePreview({ invoice, profile, preview }: { invoice: GeneratedVatInvoice | null; profile: BusinessProfile; preview: Omit<VatInvoiceDraft, "lines"> & { invoiceNumber: string; lines: ReturnType<typeof calculateInvoiceLine>[]; netTotalLkr: number; vatTotalLkr: number; grossTotalLkr: number } }) {
  ...
  return <article className="tax-invoice-preview"><header><div><span>TAX INVOICE</span><h2>{profile.legalName}</h2><p>{profile.address}</p><p>TIN <span className="mono">{profile.tin}</span> · {profile.contactPhone}</p></div><div><strong className="mono">{shown.invoiceNumber}</strong><small>Invoice <span className="mono">{shown.invoiceDate}</span></small><small>Supply <span className="mono">{shown.supplyDate}</span></small></div></header><section className="invoice-parties"><div><small>SUPPLIER</small><strong>{profile.legalName}</strong><span>TIN <span className="mono">{profile.tin}</span></span><span>{profile.address}</span></div><div><small>PURCHASER</small><strong>{shown.purchaserName || "Purchaser name"}</strong><span>TIN <span className="mono">{shown.purchaserTin || "000000000"}</span></span><span>{shown.purchaserAddress || "Registered address"}</span></div></section><table><thead><tr><th>Description</th><th>Qty</th><th>Unit LKR</th><th>Net LKR</th><th>VAT</th></tr></thead><tbody>{shown.lines.map((line, index) => <tr key={index}><td>{line.description || "Supply description"}</td><td className="mono">{line.quantity}</td><td className="mono">{money(line.unitPriceLkr)}</td><td className="mono">{money(line.netAmountLkr)}</td><td className="mono">{money(line.vatAmountLkr)}</td></tr>)}</tbody></table><footer><div><span>Place of supply: {shown.placeOfSupply || "—"}</span><span>Payment: {shown.paymentMode || "—"}</span><span>Values stated in Sri Lankan Rupees (LKR)</span></div><dl><div><dt>Net value</dt><dd className="mono">{money(shown.netTotalLkr)}</dd></div><div><dt>VAT</dt><dd className="mono">{money(shown.vatTotalLkr)}</dd></div><div className="total"><dt>Total</dt><dd className="mono">{money(shown.grossTotalLkr)}</dd></div></dl></footer><p className="invoice-draft-note">{invoice ? "Generated by ComplyPilot · Review before issuing" : "Live preview · save to assign the final serial"}</p></article>;
}
