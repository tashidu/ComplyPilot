...
export function VatRegistrationView({ workspace, profile, onSave, onOpenProfile, onConfirmRegistration }: {
  ...
  return (
    <>
      <PageHead eyebrow="IRD registration preparation" title="VAT Registration Assistant" lead="A guided TIN → e-Services → VAT tax-type workflow built from the IRD 2026 taxpayer-registration guide and TPR 005 form." action={<span className={`tag ${readiness.ready ? "ok" : "warn"}`}><span className="mono">{readiness.percentage}%</span> prepared</span>} />
      ...
      <div className="registration-prerequisites">
        <button className={/^\d{9}$/.test(profile.tin) ? "done" : ""} onClick={onOpenProfile}><b>1</b><span><strong>Get TIN</strong><small>{profile.tin ? <>TIN <span className="mono">{profile.tin}</span></> : "Add your nine-digit TIN"}</small></span></button>
      ...
      {step === 0 ? <section className="card pad registration-card">
        ...
        <div className="workspace-form-grid">
          ...
          <label><span>Taxable supplies — latest quarter (LKR)</span><input className="mono" type="number" min="0" value={draft.taxableSuppliesLastQuarterLkr} onChange={(event) => update("taxableSuppliesLastQuarterLkr", Number(event.target.value))} /></label>
          <label><span>Estimated taxable supplies — next 12 months (LKR)</span><input className="mono" type="number" min="0" value={draft.estimatedTaxableSuppliesNext12MonthsLkr} onChange={(event) => update("estimatedTaxableSuppliesNext12MonthsLkr", Number(event.target.value))} /></label>
        ...
        <div className={`eligibility-result ${turnover.mandatory ? "mandatory" : "optional"}`}><strong>{turnover.mandatory ? "Mandatory threshold may be met" : "Threshold not indicated by these values"}</strong><p>IRD currently states over <span className="mono">{money(turnover.thresholds.quarterLkr)}</span> per quarter or <span className="mono">{money(turnover.thresholds.twelveMonthsLkr)}</span> in 12 months for {activity === "FINANCIAL_SERVICES" ? "the supply of financial services" : "general goods and services"}. Importers and exporters of commercial goods must register regardless of turnover, and voluntary registration may still be possible.</p><small>Decision support only · confirm timing and classification with IRD or a tax professional.</small></div>
      ...
      {step === 1 ? <section className="card pad registration-card">
        ...
        <div className="workspace-form-grid">
          <label><span>Premises number</span><input className="mono" value={draft.premisesNo} onChange={(event) => update("premisesNo", event.target.value)} /></label>
          <label><span>Unit number (if any)</span><input className="mono" value={draft.unitNo} onChange={(event) => update("unitNo", event.target.value)} /></label>
          ...
          <label><span>Postal code</span><input className="mono" value={draft.postalCode} onChange={(event) => update("postalCode", event.target.value)} /></label>
          ...
          <label><span>Activity code (if known)</span><input className="mono" value={draft.activityCode} onChange={(event) => update("activityCode", event.target.value)} /></label>
          <label><span>Requested effective date</span><input className="mono" type="date" value={draft.requestedEffectiveDate} onChange={(event) => update("requestedEffectiveDate", event.target.value)} /></label>
          <label><span>First transaction date</span><input className="mono" type="date" value={draft.firstTransactionDate} onChange={(event) => update("firstTransactionDate", event.target.value)} /></label>
          <label><span>Estimated first taxable-supply date</span><input className="mono" type="date" value={draft.estimatedTaxableSupplyDate} onChange={(event) => update("estimatedTaxableSupplyDate", event.target.value)} /></label>
          <label><span>Total taxable supplies to date (LKR)</span><input className="mono" type="number" min="0" value={draft.taxableSuppliesToDateLkr} onChange={(event) => update("taxableSuppliesToDateLkr", Number(event.target.value))} /></label>
          ...
          <label><span>Signatory NIC / passport</span><input className="mono" value={draft.signatoryNic} onChange={(event) => update("signatoryNic", event.target.value)} /></label>
      ...
      {step === 3 ? <section className="card pad registration-card">
        ...
        {profile.vatRegistrationStatus !== "ACTIVE" ? <div className="registration-confirm"><div><span className="eyebrow">After IRD approves the registration</span><h3>Record the result</h3><p>This does not call RAMIS. It stores the confirmation reference supplied by the user.</p></div><label><span>Effective date</span><input className="mono" type="date" value={effectiveDate} onChange={(event) => setEffectiveDate(event.target.value)} /></label><label><span>Certificate / acknowledgement reference</span><input className="mono" value={certificateReference} onChange={(event) => setCertificateReference(event.target.value)} /></label><button className="button primary" disabled={!effectiveDate || certificateReference.trim().length < 3 || saving || !onConfirmRegistration} onClick={async () => { if (!onConfirmRegistration) return; setSaving(true); try { await onConfirmRegistration(effectiveDate, certificateReference); } finally { setSaving(false); } }}>Mark IRD registration confirmed</button></div> : null}
